...
new_content = """export default function Dashboard() {
...
# Replace the beginning of function implementation
dashboard_jsx = re.sub(
    r'export default function Dashboard\(\) \{.*?<h2 className="section-title">Deal Pipeline</h2>\s*<div className="pipeline-tabs">',
    new_content,
    dashboard_jsx,
    flags=re.DOTALL
)

# Remove the old properties section at the bottom!
dashboard_jsx = re.sub(
    r'\{/\* Properties Display \*/\}.*?(?=</section>\s*</div>\s*\);\s*})',
    '',
    dashboard_jsx,
    flags=re.DOTALL
)
...
# Now update the CSS
with open("src/components/Dashboard.css", "r", encoding="utf-8") as f:
...
.redfin-hero-background {
  ...
  width: 100%;
  height: 100%;
  background-image: linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.6)), url('https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80');
...
dashboard_css = re.sub(
    r'/\* Hero Search Section \*/.*?/\* Pipeline Tabs \*/',
    redfin_css + "\n/* Pipeline Tabs */",
    dashboard_css,
    flags=re.DOTALL
)

# Remove old Properties Section styles
dashboard_css = re.sub(
...
with open("src/components/Dashboard.css", "w", encoding="utf-8") as f:
